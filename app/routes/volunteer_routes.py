...
def require_volunteer_access(f):
    from functools import wraps
    @wraps(f)
    def decorated(*args, **kwargs):
        if not current_user.is_authenticated:
            abort(401)
        if not (current_user.has_role('Admin') or current_user.can('access_volunteer')):
            abort(403)
...
class DashboardView(MethodView):
    decorators = [login_required, require_volunteer_access]
